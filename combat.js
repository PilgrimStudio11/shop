...
export function getEntityPower(entity) {
    ...
    const hullPercent = Math.max(0, entity.hull) / 100;
...
export function resolveCombat(attacker, defender, isPlayerAttacker, eventFight = false) {
    ...
    if (attacker.cargo.rockets && attacker.cargo.rockets > 0 && remaining > 0) {
        const maxRocketPower = attacker.cargo.rockets * CONFIG.ROCKET_POWER;
    ...
    if (remaining > 0 && attacker.darkMatter > 0) {
        const maxTMPower = attacker.darkMatter * CONFIG.TM_POWER;
    ...
    // Урон прочности
    if (remaining > 0) {
        const basePower = getShip(attacker).power;
        hullDamage = (remaining / basePower) * 100;
        attacker.hull -= hullDamage;
        if (attacker.hull < 0) attacker.hull = 0;
    }

    // Проверка гибели атакующего
    if (attacker.hull <= 0) {
        // Атакующий погиб, меняем роли для захвата
        [attacker, defender] = [defender, attacker];
        const stolenCredits = Math.floor(defender.credits);
        attacker.credits += stolenCredits;
        defender.credits = 0;
        const stolenGoods = [];
        for (let g of window.GOODS) {
            const qty = defender.cargo[g.id] || 0;
            if (qty > 0) {
                const stolen = Math.floor(qty * (0.1 + Math.random() * 0.2));
                if (stolen > 0) {
                    const ship = getShip(attacker);
                    const currentCargo = Object.values(attacker.cargo).reduce((a,b)=>a+b,0);
                    const free = ship.cargo - currentCargo;
                    const actual = Math.min(stolen, Math.floor(free / g.cargoSpace));
                    if (actual > 0) {
        ...
        if (attacker === player) {
            ...
            // Освобождение планет бота уже в removeBot
        }
        return true;
    }
    ...
    if (attacker === player) {
        ...
        if (eventFight && defender.name === "Пират-изгой") {
            const extra = randomRange(200, 500);
...
export function fightWithBot(bot, eventFight = false) {
    ...
    const canAttack = (Math.abs(player.level - bot.level) <= 1) || player.ignoreLevelOnce || bot.ignoreLevelOnce;
    if (!canAttack) {
...
function autoActivateArtifacts(entity, isPlayer) {
    if (isPlayer) {
        if (Math.abs(player.level - entity.level) > 1 && hasArtifact(player.artifacts, "amuletSupremacy")) {
...
export function encounterWithBots() {
    ...
    const botsHere = window.bots.filter(b => b.currentPlanet === player.currentPlanet);
    ...
    const canFight = (Math.abs(player.level - bot.level) <= 1) || player.ignoreLevelOnce || bot.ignoreLevelOnce;
    if (!canFight) {
