...
import { player, getShip } from './player.js';
...
export function triggerRandomEvent() {
    ...
    if (r < 0.15 && Math.random() < CONFIG.EVENT_HELP_SHIP_CHANCE / 0.03) {
        ...
        if (choice) {
            ...
            const tempBot = { ...createBot(1, 0), name: "Пират-изгой", credits: randomRange(500, 1500), hull: 80, cargo: {}, shipLevel: 1 };
            for (let g of GOODS) tempBot.cargo[g.id] = randomRange(0, 5);
            fightWithBot(tempBot, true);
        }
    } else if (r < 0.35) {
